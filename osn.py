...
__all__ = ['OSNSite', 'ImageSoftware', 'Instrument', 'ApogeeAltaU9',
           'MaximDL4', 'MaximDL5']


class OSNSite(EarthLocation):
    """
    The Sierra Nevada Observatory site.

    An astropy location with the observatory location pre-set to:

        + `lat` = 37.064166666666665 degrees North
        + `long` = 3.3847222222222224 degrees West
        + `height` = 2896.0 meters

    and a few additional properties/methods that are convenient:

        + `name` = Sierra Nevada Observatory (OSN)
    """

    def __new__(cls):
        return EarthLocation.__new__(OSNSite,
                                     lat=37.064167*u.degree,
                                     lon=3.3847222222222224*u.degree,
                                     height=2896.0*u.m)

    def __init__(self):
        self._name = 'Sierra Nevada Observatory (OSN)'
...
class OSN(object):
    """
    Class encapsulating site, instrument, and software information for OSN
    Observatory.

    Attributes
    ----------
    site : osn.OSNSite instance
    instrument : dict
