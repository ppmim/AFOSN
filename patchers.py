...
# Copyright (c) 2017 Jose M. Ibanez All rights reserved.
...
def add_time_info(header, history=False):
    ...
    osn.JD_OBS.value = dateobs.jd
    osn.MJD_OBS.value = dateobs.mjd

    LST_tmp = _lst_from_obstime(dateobs)

    osn.LST.value = LST_tmp.to_string(unit=u.hour, sep=':', precision=4,
                                        pad=True)

    for keyword in osn.keywords_for_all_files:
...
def add_object_pos_airmass(header, history=False):
    ...
    Notes
    -----
    Has side effect of setting OSN site JD to JD-OBS, which means it
    also assume JD.value has been set.

    """
    # not sure why coverage is not picking up both branches, but it is not, so
    # marking it no cover
    if osn.JD_OBS.value is None:   # pragma: no cover
        raise ValueError('Need to set JD_OBS.value '
                         'before calling.')

    try:
        osn.RA.set_value_from_header(header)
    except ValueError:
        raise ValueError("No RA is present.")

    osn.DEC.set_value_from_header(header)
    osn.RA.value = osn.RA.value.replace(' ', ':')
    osn.DEC.value = osn.DEC.value.replace(' ', ':')

    obj_coord2 = SkyCoord(osn.RA.value, osn.DEC.value,
                          unit=(u.hour, u.degree), frame='fk5')

    obstime = Time(osn.MJD_OBS.value, format='mjd')
    alt_az = obj_coord2.transform_to(AltAz(obstime=obstime,
                                           location=osn.site))

    osn.ALT_OBJ.value = round(alt_az.alt.degree, 5)
    osn.AZ_OBJ.value = round(alt_az.az.degree, 5)
    osn.AIRMASS.value = round(1 / np.cos(np.pi / 2 - alt_az.alt.radian), 3)
    ...
    osn.HA.value = HA.to_string(unit=u.hour, sep=':')

    for keyword in osn.keywords_for_light_files:
...
def get_software_name(header, file_name=None, use_observatory=None):
    ...
    msumastro.osn.Software object
    """
    fits_file = file_name or ''
    observatory = use_observatory or osn
...
def add_image_unit(header, history=True):
    ...
    instrument = osn.instruments[header[instrument_key]]
...
def add_overscan_header(header, history=True):
    ...
    instrument = osn.instruments[header['instrume']]
    overscan_present = instrument.has_overscan(image_dim)
    modified_keywords = []
    if overscan_present:
        overscan_region = osn.BIASSEC
        trim_region = osn.TRIMSEC
...
def add_ra_dec_from_object_name(directory=None, new_file_ext=None,
    ...
    for object_name in objects:  # pragma: nobranch
        ...
        common_format_keywords = {'sep': ':',
                                  'precision': 2,
                                  'pad': True}
        osn.RA.value = object_coords.ra.to_string(unit=u.hour,
                                                    **common_format_keywords)
        osn.DEC.value = object_coords.dec.to_string(unit=u.degree,
                                                      alwayssign=True,
                                                      **common_format_keywords)
        ...
        for image in these_files:
            ...
            osn.RA.add_to_header(header, history=True)
            osn.DEC.add_to_header(header, history=True)
            if new_file_ext:
